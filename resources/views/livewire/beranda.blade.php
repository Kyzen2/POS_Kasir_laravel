<style>
    .div{
        margin: 2;
    }
</style>


<div class="container py-5">
    <!-- Hero Section -->
    <div class="row mb-4">
        <div class="col-12 text-center">
            <h1 class="display-4 fw-bold text-primary">Selamat datang di POS Kasir</h1>
            <p class="lead text-muted">Temukan solusi terbaik untuk setiap kebutuhan Anda di sini. Jelajahi produk dan layanan kami yang dapat mempercepat pencapaian tujuan Anda..</p>
            <a href="#produk-unggulan" class="btn btn-primary btn-lg mt-3 btn-custom">Lihat Produk</a>
        </div>
    </div>

    <style>
        /* Custom Button Styling */
        .btn-custom {
            padding: 12px 30px;
            font-size: 1.2rem;
            font-weight: 600;
            border-radius: 30px;
            background: linear-gradient(135deg, rgba(38, 0, 255, 0.77), rgba(183, 0, 255, 0.64));
            border: none;
            color: white;
            box-shadow: 0 5px 15px rgba(0, 0, 0, 0.17);
            transition: all 0.4s ease;
        }

        /* Hover Effect */
        .btn-custom:hover {
            background: linear-gradient(135deg, rgb(17, 0, 255), #00a3cc);
            transform: translateY(-5px);
            box-shadow: 0 10px 20px rgba(0, 0, 0, 0.29);
        }

        /* Focus Effect */
        .btn-custom:focus {
            outline: none;
        }
    </style>


    <!-- Produk Unggulan -->
    <div id="produk-unggulan" class="container mt-5">
        <h2 class="text-center fw-bold mb-4">Produk Unggulan Kami</h2>
        <p class="text-center text-muted mb-5">Temukan produk terbaik pilihan kami, hanya untuk Anda.</p>
        <div class="row">
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100 shadow border-0">
                    <img src="https://via.placeholder.com/350" class="card-img-top" alt="Produk 1">
                    <div class="card-body d-flex flex-column">
                        <h5 class="card-title">Produk 1</h5>
                        <p class="card-text">Deskripsi singkat tentang produk ini. Fitur-fitur unggulan yang membuatnya layak untuk dicoba.</p>
                        <a href="#" class="btn btn-primary mt-auto">Lihat Detail</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100 shadow border-0">
                    <img src="https://via.placeholder.com/350" class="card-img-top" alt="Produk 2">
                    <div class="card-body d-flex flex-column">
                        <h5 class="card-title">Produk 2</h5>
                        <p class="card-text">Deskripsi singkat tentang produk ini. Fitur-fitur unggulan yang membuatnya layak untuk dicoba.</p>
                        <a href="#" class="btn btn-primary mt-auto">Lihat Detail</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100 shadow border-0">
                    <img src="https://via.placeholder.com/350" class="card-img-top" alt="Produk 3">
                    <div class="card-body d-flex flex-column">
                        <h5 class="card-title">Produk 3</h5>
                        <p class="card-text">Deskripsi singkat tentang produk ini. Fitur-fitur unggulan yang membuatnya layak untuk dicoba.</p>
                        <a href="#" class="btn btn-primary mt-auto">Lihat Detail</a>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Testimoni Pengguna -->
    <div class="container mt-5">
        <h2 class="text-center fw-bold mb-4">Apa Kata Pengguna?</h2>
        <p class="text-center text-muted mb-5">Kami bangga mendengar pengalaman positif dari pengguna kami.</p>
        <div class="row">
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100 border-0 shadow text-center testimonial-card">
                    <div class="card-body d-flex flex-column">
                        <p class="card-text mb-4">"Produk ini benar-benar mempermudah dan meningkatkan efisiensi kerja kami. Terima kasih atas layanan yang luar biasa!"</p>
                        <h5 class="card-title mb-0">Elaina</h5>
                        <small class="text-muted">Penyihir Abu</small>
                        <div class="mt-auto">
                            <i class="fas fa-quote-left fa-2x text-primary opacity-25"></i>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100 border-0 shadow text-center testimonial-card">
                    <div class="card-body d-flex flex-column">
                        <p class="card-text mb-4">"Produk dengan kualitas terbaik dan layanan pelanggan yang profesional. Benar-benar mengesankan!"</p>
                        <h5 class="card-title mb-0">Ace Anthem</h5>
                        <small class="text-muted">Streamers</small>
                        <div class="mt-auto">
                            <i class="fas fa-quote-left fa-2x text-primary opacity-25"></i>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100 border-0 shadow text-center testimonial-card">
                    <div class="card-body d-flex flex-column">
                        <p class="card-text mb-4">"Pengalaman yang sangat memuaskan! Proses pemesanan dan pengiriman berjalan sangat cepat dan lancar."</p>
                        <h5 class="card-title mb-0">Wielino ino</h5>
                        <small class="text-muted">Streamers</small>
                        <div class="mt-auto">
                            <i class="fas fa-quote-left fa-2x text-primary opacity-25"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Keunggulan Kami -->
    <div class="row mb-5">
        <div class="col-12 text-center">
            <h2 class="fw-bold text-primary">Kenapa Memilih Kami?</h2>
            <p class="text-muted">Berikut adalah beberapa alasan mengapa kami menjadi pilihan tepat.</p>
        </div>
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="card shadow-lg rounded-lg border-0 transform-hover">
                <div class="card-body text-center">
                    <h5 class="card-title text-danger">Kualitas Terjamin</h5>
                    <p class="card-text">Kami selalu menjaga kualitas produk dan layanan kami untuk kepuasan pelanggan.</p>
                </div>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="card shadow-lg rounded-lg border-0 transform-hover">
                <div class="card-body text-center">
                    <h5 class="card-title text-warning">Pengiriman Cepat</h5>
                    <p class="card-text">Proses pengiriman yang cepat dan tepat waktu untuk kenyamanan Anda.</p>
                </div>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="card shadow-lg rounded-lg border-0 transform-hover">
                <div class="card-body text-center">
                    <h5 class="card-title text-success">Layanan Pelanggan 24/7</h5>
                    <p class="card-text">Tim layanan pelanggan kami siap membantu Anda kapan saja dengan solusi yang tepat.</p>
                </div>
            </div>
        </div>
    </div>

    <!-- CSS -->
    <style>
        .row {
            margin-top: 50px;
        }

        .fw-bold {
            font-weight: 700;
        }

        .card {
            transition: transform 0.3s ease, box-shadow 0.3s ease;
            background: linear-gradient(135deg, rgba(0, 140, 255, 0.5), rgba(183, 0, 255, 0.57));
            color: white;
            border-radius: 15px;
        }

        .card:hover {
            transform: translateY(-10px);
            box-shadow: 0px 20px 50px rgba(0, 0, 0, 0.1);
        }

        .card-body {
            padding: 2rem;
        }

        .card-title {
            font-size: 1.25rem;
            font-weight: 700;
            margin-bottom: 10px;
        }

        .card-text {
            font-size: 1rem;
            line-height: 1.5;
        }

        .transform-hover:hover {
            transform: scale(1.05);
        }

        /* Text colors for each card */
        .text-danger {
            color: rgb(65, 68, 255);
        }

        .text-warning {
            color: #ffb74d;
        }

        .text-success {
            color: #66bb6a;
        }

        /* Header styling */
        h2.text-primary {
            color: #0062cc;
        }
    </style>

    <!-- Footer -->
    <footer class="text-center py-4 bg-dark text-white">
        <p>&copy; 2025 Turky. All rights reserved.</p>
        <div>
            <a href="#" class="text-white me-3">Privacy Policy</a>
            <a href="#" class="text-white me-3">Terms of Service</a>
            <a href="mailto:turkykw50@gmail.com" class="text-white">Kontak Kami</a>
        </div>
    </footer>

    {{-- <div>

</div> --}}